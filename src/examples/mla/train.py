...
torchrun --standalone --nproc-per-node=1 src/examples/mla/train.py run_name [OVERRIDES...]
torchrun --nproc-per-node=4 src/examples/mla/train.py run_name [OVERRIDES...]
torchrun --standalone --nproc-per-node=1 src/examples/mla/train.py run_name trainer.save_folder=/home/billy/Develop/OLMo-checkpoints/my_run
"""
#import os
import sys
...
def build_config(run_name: str, overrides: List[str]) -> ExperimentConfig:
    ...
    model_config = TransformerConfig.mla_small(
        vocab_size=tokenizer_config.padded_vocab_size(),  # a little bigger than actual vocab size to make it a multiple of 128
        use_dsa=False #choose to use the top-k indexer here. 
    )
