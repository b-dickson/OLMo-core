...
#SBATCH -J attn-hgdn-hc-h100
#SBATCH --gres=gpu:H100:2
#SBATCH --mem=256G
#SBATCH --partition general
#SBATCH --output=/data/user/dicksonb/logs/attn-ladder-hgdn-hc/%A_%a.txt
#SBATCH --error=/data/user/dicksonb/logs/attn-ladder-hgdn-hc/%A_%a.err
#SBATCH --mail-type=None
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --time=48:00:00
# Submit with: sbatch --array=1-4 src/scripts/lair/slurm_wsds_hybrid_gated_deltanet_hc_h100.sh

CONFIG_FILE="${CONFIG_FILE:-src/scripts/lair/wsds_hybrid_gated_deltanet_hc.txt}"
...
RUN_NAME="${RUN_NAME:-hc${HC_STREAMS}-${ATTENTION_TYPE}-${CHINCHILLA_MULTIPLE}x-${SIZE}_seq${SEQUENCE_LENGTH}_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}_${RUN_OPTIMIZER}}"

echo "Variant: hybrid_gated_deltanet + Identity Hyper-Connections (n_streams=${HC_STREAMS}); 3:1 GatedDeltaNet:full softmax per 4-layer block"
echo "Attention type: ${ATTENTION_TYPE}"
echo "Size: ${SIZE}"
echo "Chinchilla multiple: ${CHINCHILLA_MULTIPLE}"
echo "Sequence length: ${SEQUENCE_LENGTH}"
echo "Sliding window size: ${SLIDING_WINDOW_SIZE} (unused for hybrid_gated_deltanet)"
echo "Hyper connections: ${HC_STREAMS} streams"
echo "Optimizer: ${RUN_OPTIMIZER}"
echo "Run name: ${RUN_NAME}"
mkdir -p "/data/user/dicksonb/logs/attn-ladder-hgdn-hc"
