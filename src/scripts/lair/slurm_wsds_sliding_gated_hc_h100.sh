...
#SBATCH -J attn-sliding-gated-hc-h100
#SBATCH --gres=gpu:H100:2
#SBATCH --mem=256G
#SBATCH --partition general
#SBATCH --output=/data/user/dicksonb/logs/attn-ladder-sliding-gated-hc/%A_%a.txt
#SBATCH --error=/data/user/dicksonb/logs/attn-ladder-sliding-gated-hc/%A_%a.err
#SBATCH --mail-type=None
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --time=72:00:00
# Submit with: sbatch --array=1-4 src/scripts/lair/slurm_wsds_sliding_gated_hc_h100.sh

CONFIG_FILE="${CONFIG_FILE:-src/scripts/lair/wsds_sliding_gated_hc.txt}"
...
echo "Variant: sliding_gated + Identity Hyper-Connections (n_streams=${HC_STREAMS}); 3:1 sliding(${SLIDING_WINDOW_SIZE}):full(${SEQUENCE_LENGTH}) per 4-layer block"
...
mkdir -p "/data/user/dicksonb/logs/attn-ladder-sliding-gated-hc"
