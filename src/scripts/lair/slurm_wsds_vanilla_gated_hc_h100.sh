...
#SBATCH -J attn-vanilla-gated-hc-h100
#SBATCH --gres=gpu:H100:2
#SBATCH --mem=256G
#SBATCH --partition general
#SBATCH --output=/data/user/dicksonb/logs/attn-ladder-vanilla-gated-hc/%A_%a.txt
#SBATCH --error=/data/user/dicksonb/logs/attn-ladder-vanilla-gated-hc/%A_%a.err
#SBATCH --mail-type=None
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --time=72:00:00
# Submit with: sbatch --array=1-4 src/scripts/lair/slurm_wsds_vanilla_gated_hc_h100.sh

CONFIG_FILE="${CONFIG_FILE:-src/scripts/lair/wsds_vanilla_gated_hc.txt}"
...
echo "Variant: vanilla_gated + Identity Hyper-Connections (n_streams=${HC_STREAMS})"
echo "Attention type: ${ATTENTION_TYPE}"
echo "Size: ${SIZE}"
echo "Chinchilla multiple: ${CHINCHILLA_MULTIPLE}"
echo "Sequence length: ${SEQUENCE_LENGTH}"
echo "Sliding window size: ${SLIDING_WINDOW_SIZE} (unused for vanilla_gated)"
echo "Hyper connections: ${HC_STREAMS} streams"
echo "Optimizer: ${RUN_OPTIMIZER}"
echo "Run name: ${RUN_NAME}"
mkdir -p "/data/user/dicksonb/logs/attn-ladder-vanilla-gated-hc"
